...
// ---------------------------------------------------------------------------
// Legacy renderVaultNote — kept for backward compatibility with templates.test.ts
// Wraps buildVaultNote with the old `mode`-based interface.
// ---------------------------------------------------------------------------

interface LegacyVaultNoteInput extends VaultNoteInput {
  mode: CaptureMode | "full";
}

export function renderVaultNote(input: LegacyVaultNoteInput): string {
  if (input.type === "youtube-channel") {
...
function renderSourceNote(input: LegacyVaultNoteInput): string {
  const frontmatter = renderFrontmatter(input);
  const body = renderBody(input);
  return `---\n${frontmatter}---\n\n# ${input.title}\n\n${body}`;
}

function renderYouTuberNote(input: LegacyVaultNoteInput): string {
  const lines = [
    "---",
    "categories:",
    '  - "[[YouTubers]]"',
...
function renderFrontmatter(input: LegacyVaultNoteInput): string {
  const lines: string[] = [];
...
function renderBody(input: LegacyVaultNoteInput): string {
  if (input.mode === "quick") {
